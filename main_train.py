...
def main():
    ...
    RESUME_MODEL = "models/Sparky_run_1_49000000.zip"

    if os.path.exists(RESUME_MODEL):
        print(f"🔄 SBLOCCO RUN {config.RUN_NUMBER} DA {RESUME_MODEL}")
        model = PPO.load(RESUME_MODEL, env=envs, device="cuda",
                         custom_objects={"learning_rate": 5e-5, "ent_coef": 0.05, "n_steps": 8192, "batch_size": 2048})
    else:
    ...
    model.learn(total_timesteps=50_000_000, callback=[checkpoint, vision],
